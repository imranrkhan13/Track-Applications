...
import {
    ...
    ExternalLink, FileSearch, Flag, Lightbulb, Link2, LoaderCircle, Mic, PanelTop, RefreshCw, Search, Sparkles, Target,
} from "lucide-react";
...
export default function RoleRoom({ jobs, selectedJob, setSelectedJob, onMock, userId = "demo-user" }) {
    ...
    return <div className="page-grid role-room-page"><RoleSelector jobs={jobs} jobId={jobId} onChange={value => { setJobId(value); setSelectedJob(jobs.find(item => item.id === value)); }} /><RoomHero job={job} stage={stage} taskCount={taskCount} completedCount={completedCount} onMock={onMock} /><RoomTabs tab={tab} setTab={setTab} />{tab === "research" && <ResearchTab job={job} notes={notes} setNotes={onNotesChange} analysis={analysis} analysisState={analysisState} analysisError={analysisError} onAnalyze={runAnalysis} />}{tab === "plan" && <PlanTab job={job} completed={completed} onToggle={id => update({ ...completed, [id]: !completed[id] })} analysis={analysis} />}{tab === "questions" && <QuestionsTab job={job} onMock={onMock} analysis={analysis} />}{tab === "notes" && <NotesTab job={job} notes={notes} setNotes={onNotesChange} onSave={saveNotes} />}</div>;
}
