...
function Sidebar({ page, setPage, jobs, user, onSignOut, collapsed, onToggle }) {
    ...
    const renderItem = ([id, label, NavIcon]) => <button type="button" key={id} className={page === id ? "nav-item active" : "nav-item"} onClick={() => setPage(id)} aria-label={label} title={collapsed ? label : undefined}><Icon>{React.createElement(NavIcon, { size: 17 })}</Icon><span className="nav-label">{label}</span>{counts[id] ? <em>{counts[id]}</em> : null}</button>;
    return <aside className={`sidebar-new compact-sidebar sidebar-v4 ${collapsed ? "is-collapsed" : ""}`}>
        <div className="sidebar-top sidebar-v4-top"><div className="brand brand-v4"><div className="brand-mark brand-mark-v4"><Sprout size={18} /></div><div className="brand-copy"><strong>Career Garden</strong><small>job search, cultivated</small></div></div><button type="button" className="sidebar-collapse-toggle" onClick={onToggle} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} title={collapsed ? "Expand sidebar" : "Collapse sidebar"}>{collapsed ? <PanelLeftOpen size={16} /> : <PanelLeftClose size={16} />}</button></div>
        <div className="sidebar-season-v4"><span className="sidebar-season-icon"><Leaf size={15} /></span><span><small>Current season</small><b>{currentStage.title}</b></span><em>{cycleProgress}%</em></div>
        <nav className="side-nav sidebar-v4-nav"><div className="nav-group"><p>Your workspace</p>{primary.map(renderItem)}</div><section className="sidebar-cycle-v4 sidebar-cycle-simple" aria-label="Current plant stage"><div className="sidebar-cycle-v4-head"><div><span>Plant cycle</span><b>{currentStage.label}</b></div><em>{currentStage.step} / 06</em></div><div className="sidebar-cycle-current"><span className="sidebar-cycle-v4-node" style={{ background: currentStage.tint, color: currentStage.color }}><StageIcon stage={currentStage} size={15} /></span><span><b>{currentStage.title}</b><small>{currentJob ? "Your next move is in motion" : "Add a role to begin"}</small></span></div><div className="sidebar-cycle-v4-foot"><span><i style={{ width: `${cycleProgress}%` }} /></span><small>{cycleProgress}% of the plant cycle</small></div></section></nav>
...
function Applications({ jobs, openJob, editJob, addJob }) {
    ...
    return <div className="page-grid applications-simple">
        <section className="applications-simple-head"><div><span className="section-kicker">Your roles</span><h2>Keep every opportunity in one place.</h2><p>Add the role first. Then open it whenever you are ready to research or practice.</p></div><button className="primary-btn" onClick={addJob}><Plus size={16} />Add role</button></section>
        ...
        <section className="panel simple-roles-list" aria-label="Saved roles">
            {filtered.length ? filtered.map(job => <div role="button" tabIndex="0" className="simple-role-row" key={job.id} onClick={() => openJob(job)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openJob(job); } }}><span className="company-logo">{job.company.slice(0, 1)}</span><span className="simple-role-copy"><strong>{job.role}</strong><small>{job.company} · {job.location || "Location not set"}</small></span><span className="simple-role-stage"><StagePill status={job.status} /><small>{job.next_step || "Ready for the next move"}</small></span><select value={job.status} aria-label={`Change ${job.company} stage`} onClick={event => event.stopPropagation()} onChange={event => editJob({ ...job, status: event.target.value })}>{STAGES.map(stage => <option key={stage}>{stage}</option>)}</select><ArrowRight size={17} className="simple-role-arrow" /></div>) : <EmptyState title="Your garden is ready for a role." description="Start with the company and role. Everything else can come later." action="Add role" onAction={addJob} />}
        </section>
    </div>;
}
...
export default function Workspace({ user = DEMO_USER, onSignOut }) {
    ...
    useEffect(() => { getJobs(user.id).then(data => { setJobs(data); setLoading(false); }).catch(error => { setJobs([]); setLoadError(error?.message || "Could not load your workspace data."); setLoading(false); }); }, [user.id]);
    ...
    const save = async job => { const isNew = !job.id; if (!job.company?.trim() || !job.role?.trim()) { setActionError("Add the company and role to plant this opportunity."); return; } setActionError(""); try { const saved = await saveJob(job, user.id); setJobs(current => [saved, ...current.filter(item => item.id !== saved.id)]); setDrawer(null); if (isNew) setPage("overview"); } catch (error) { setActionError(error?.message || "We could not save this role."); } };
    const remove = async id => { if (!id) return; setActionError(""); try { await deleteJob(id, user.id); setJobs(current => current.filter(job => job.id !== id)); setDrawer(null); } catch (error) { setActionError(error?.message || "We could not delete this application."); } };
