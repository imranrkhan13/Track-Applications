...
function StagePill({ status }) { const stage = stageInfo(status); return <span className="stage-pill plant-stage-pill" style={{ background: stage.tint, color: stage.color }}><span className="stage-dot" />{stage.gardenName || status}</span>; }

function MiniPipeline({ jobs, onStage }) { const counts = PLANT_STAGES.map(stage => ({ ...stage, count: jobs.filter(job => job.status === stage.id).length })); const current = counts.find(item => item.count > 0 && item.id !== "Rejected") || counts[0]; return <section className="panel pipeline-panel plant-pipeline-panel"><div className="panel-heading"><div><p className="eyebrow">Your garden</p><h2>The search is growing</h2></div><button className="text-btn" onClick={() => onStage("applications")}>Tend all <ArrowRight size={14} /></button></div><div className="plant-pipeline-track">{counts.map(stage => <button className={`plant-pipeline-node ${stage.id === current.id ? "current" : ""}`} key={stage.id} onClick={() => onStage("applications")} title={`View ${stage.gardenName || stage.label} applications`}><span className="pipeline-plant">{React.createElement(StageIcon, { stage, size: 20 })}</span><span className="pipeline-node-line" /> <b>{stage.count}</b><small>{stage.gardenName || stage.label}</small><em>{stage.statusText || stage.label}</em></button>)}</div><div className="pipeline-message"><span className="pipeline-message-icon">{React.createElement(StageIcon, { stage: current, size: 21 })}</span><div><b>{current.gardenName || current.title}</b><small>{current.short}</small></div><button onClick={() => onStage(current.id === "Interview" ? "mock" : "applications")}>{current.id === "Interview" ? "Practice now" : "View this stage"} <ArrowRight size={13} /></button></div></section>; }
