...
export default function CaptureSection({ onStart }) {
    ...
    return <section className="cg-section cg-capture" id="capture"><div className="cg-shell cg-capture-grid">
        <Reveal className="cg-capture-copy"><span className="cg-kicker cg-kicker-light"><i />Add a role</span><h2>Paste the role.<br /><em>Build your plan.</em></h2><p>Add a public job link or paste the description. Review what Career Garden finds, then create a focused interview plan.</p><div className="cg-url-input"><input value={url} onChange={event => setUrl(event.target.value)} aria-label="Job URL" /><button type="button" onClick={run} disabled={!url.trim() || (active >= 0 && active < STEPS.length - 1)}>{active >= 0 && active < STEPS.length - 1 ? <LoaderCircle className="cg-spin" size={15} /> : <ArrowRight size={15} />}Add role</button></div><small>You review every detail before it becomes part of your plan.</small></Reveal>
