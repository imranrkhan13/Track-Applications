...
function PlantCycle({ jobs, onApplications }) {
    ...
    return <section className="dashboard-card plant-cycle-card">
        ...
        <p className="plant-cycle-intro"><Leaf size={14} /> Every role moves through six small stages. Right now, your garden is in <strong>{current.label.toLowerCase()}</strong>.</p>
        <div className="plant-cycle-steps" role="list" aria-label="Six plant stages">{stages.map((stage, index) => <button type="button" role="listitem" className={`plant-cycle-step ${stage.id === current.id ? "is-current" : ""}`} key={stage.id} onClick={onApplications}><span className="plant-cycle-index">0{index + 1}</span><span className="plant-cycle-icon" style={{ color: stage.color, background: stage.tint }}><StageIcon stage={stage} size={17} /></span><strong>{stage.label}</strong><small>{stage.count ? `${stage.count} role${stage.count === 1 ? "" : "s"}` : stage.title}</small></button>)}</div>
        <div className="plant-cycle-foot"><span><i style={{ width: `${Math.max(16, ((stages.findIndex(stage => stage.id === current.id) + 1) / PLANT_STAGES.length) * 100)}%` }} /></span><small>Current stage: {current.title}</small></div>
    </section>;
}
